...
import { buildSuppressionFilter, detectPatterns, type PatternMatch } from "./patterns";
...
export function analyzeFile(code: string, file: string): PatternMatch[] {
  ...
  if (ast !== null) {
    ...
    // and splice in the AST verdicts in source order. The AST tier is built
    // outside detectPatterns, so inline suppressions are applied here instead.
