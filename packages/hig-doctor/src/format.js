...
const getFindingsForText = (result, verbose) => {
  const findings = verbose ? result.findings : result.findings.filter((finding) => finding.severity === "error")
...
export const formatTextResult = (result, options = {}) => {
  ...
  for (const finding of findings) {
    ...
    lines.push(`[${level}] [${scope}] ${finding.ruleId}: ${message}${formatLocation(result.directory, finding.file)}`)
  }
