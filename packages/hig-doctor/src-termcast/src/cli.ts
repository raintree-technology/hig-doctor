...
function bar(positives: number, concerns: number, total: number, width: number = 20): string {
  ...
  return c.green + "█".repeat(goodW) + c.dim + "░".repeat(Math.max(0, neutralW)) + c.red + "█".repeat(badW) + c.reset
}
...
`)
  ...
  s.done(
    `Scanned ${c.bold}${scanResult.codeFiles.length}${c.reset} code + ${c.bold}${scanResult.styleFiles.length}${c.reset} style files`,
  )
      ...
      JSON.stringify(
        {
          ...
          files: {
            code: scanResult.codeFiles.length,
            style: scanResult.styleFiles.length,
            config: scanResult.configFiles.length,
          },
  ...
  process.stdout.write("\n")
  process.stdout.write(
    `  ${c.bold}HIG Audit: ${appName}${c.reset}  ${severityBadge(totalCritical, totalSerious, totalModerate)}\n`,
  )
  process.stdout.write(
    `  ${c.dim}${scanResult.frameworks.join(", ")} · ${totalDetections} detections · ${scanResult.codeFiles.length + scanResult.styleFiles.length} files${c.reset}\n`,
  )
  process.stdout.write("\n")
  ...
  if (isLowDensity) {
    process.stdout.write(
      `  ${c.yellow}Low UI density${c.reset} — Few HIG-relevant patterns detected (${detectionsPerFile.toFixed(1)}/file).\n`,
    )
    process.stdout.write(
      `  ${c.dim}This project may not be UI-focused. Audit results are less meaningful with sparse data.${c.reset}\n`,
    )
  } else if (totalCritical > 0) {
    process.stdout.write(
      `  ${c.red}Critical issues found${c.reset} — Accessibility-breaking violations need immediate attention.\n`,
    )
