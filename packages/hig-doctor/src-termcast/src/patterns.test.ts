...
describe("detectPatterns — Swift", () => {
  ...
  test("detects accessibility modifiers", () => {
    ...
    expect(
      matches.filter((m) => m.category === "foundations" && m.subcategory === "accessibility").length,
    ).toBeGreaterThan(0)
  })
...
describe("regression — multi-line tags & child-aware rules", () => {
  ...
  test("flags a <video> with no <track> child", () => {
    expect(
      detectPatterns(`<video controls src="v.mp4"></video>`, "Player.tsx").some(
        (m) => m.pattern === "video without track" && m.type === "concern",
      ),
    ).toBe(true)
  })
  ...
  test("flags a multi-line bare <svg>", () => {
    const code = `<svg\n  viewBox="0 0 24 24"\n  fill="none"\n>`
    expect(detectPatterns(code, "Icon.tsx").some((m) => m.pattern === "svg without a11y" && m.type === "concern")).toBe(
      true,
    )
  })
  ...
  test("does NOT flag autoPlay={false}", () => {
    expect(
      detectPatterns(`<video autoPlay={false} src="v.mp4" />`, "Player.tsx").some(
        (m) => m.pattern === "autoplay media",
      ),
    ).toBe(false)
  })
  test("flags a bare autoPlay (JSX boolean true)", () => {
    expect(
      detectPatterns(`<video autoPlay src="v.mp4" />`, "Player.tsx").some(
        (m) => m.pattern === "autoplay media" && m.type === "concern",
      ),
    ).toBe(true)
  })
  test("flags lowercase autoplay (HTML)", () => {
    expect(
      detectPatterns(`<video autoplay src="v.mp4"></video>`, "page.html").some(
        (m) => m.pattern === "autoplay media" && m.type === "concern",
      ),
    ).toBe(true)
  })
...
describe("regression — Swift @State matcher & severity downgrades", () => {
  ...
  test("flags a non-private @State var", () => {
    expect(
      detectPatterns(`@State var counter = 0`, "V.swift").some(
        (m) => m.pattern === "non-private @State" && m.type === "concern" && m.category === "patterns",
      ),
    ).toBe(true)
  })
  test("does NOT flag @State private var", () => {
    expect(
      detectPatterns(`@State private var name = ""`, "V.swift").some((m) => m.pattern === "non-private @State"),
    ).toBe(false)
  })
  test("does NOT flag @StateObject", () => {
    expect(
      detectPatterns(`@StateObject var vm = VM()`, "V.swift").some((m) => m.pattern === "non-private @State"),
    ).toBe(false)
  })
