...
```bash
npm --prefix packages/skill-validator install
...
1. Add a `PatternRule` to the relevant framework array in [patterns.ts](packages/core/src/patterns.ts). Use a clear, unique `pattern` name — the severity classifier keys on it.
...
4. Add a matching test case to [patterns.test.ts](packages/core/src/patterns.test.ts).
5. Run `bun test` in `packages/cli/`.

### Mapping a rule to a category

Rules use `category` + `subcategory` fields. If a rule belongs to a new category, update `CATEGORY_TO_SKILL` and `CATEGORY_LABELS` in [categorizer.ts](packages/core/src/categorizer.ts).
